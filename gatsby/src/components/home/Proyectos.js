...
const Proyectos = ({data,query,resetFilter}) => {
    ...
    useEffect(() => {
      function handleResize() {
        ...
        if (w < 640) setColumnWidth(Math.floor(w / 1));
        else if (w < 1280) setColumnWidth(Math.floor(w / 2));
        else setColumnWidth(Math.floor(w / 4));
      }
      handleResize();
    ...
    return(
        <Container ref={containerRef}>
          <ResultContainer>
            {query && <div><h2 key={query}>Resultados de "{query}" ({filteredArticulos.length})</h2></div>}
            {(filteredArticulos.length===0)&&<NoSearchResults>No hay resultados</NoSearchResults>}
          </ResultContainer>
          <ProyectosContainer targetBlockWidth={columnWidth} responsive={true}>
            {filteredArticulos.map(( node ) => {
                return (
                    <XBlock width={node.destacado ? '2' : '1'}>
          ...
          <ul className='categories'>
            {data.allSanityCategoriasPage.nodes.map(( node ) => {
                return (<li><Link to={`/categorias/${node.slug.current}`}><p>{node.title}</p></Link></li>)
            })}
          </ul>
        </Container>
    )
...
const NoSearchResults=styled.div`padding: 20px;`
...
const ResultContainer = styled.div`
    ...
    text-align: center;
    h2 { border-top: solid 1px black; padding: 16px; font-size: 1rem; }
`
const ProyectosContainer = styled(XMasonry)`
    ...
    article {
        ...
        @media screen and (max-width: 650px){ padding: 1.33rem; }
        .icon { display: none; }
    ...
    .destacado {
        ...
        .texto {
            ...
            p { max-width: 80%; @media screen and (max-width: 650px){ max-width: 100%; } }
        }
    }
